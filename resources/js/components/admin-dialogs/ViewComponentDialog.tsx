...
// Utility function to validate component for preview
const isValidForPreview = (component: ComponentLibraryItem): boolean => {
  if (!component || typeof component !== 'object') return false;
  
  // Check required properties
  if (!component.type || !component.name) return false;
  
  // Valid even without SVG - the component will handle that case
  return true;
};
...
const ViewComponentDialog = ({
  ...
  // Validate component data outside render function
  const isValid = useMemo(() => isValidForPreview(component), [component]);
  
  // Create safer component copy to avoid passing potentially invalid props
  const safeComponent = useMemo(() => {
    if (!isValid) return null;
    
    // Create a sanitized version with validated fields
    return {
      ...component,
      // Ensure SVG path is either a string or null
      svgPath: typeof component.svgPath === 'string' ? component.svgPath : null,
      // Ensure pins is an array
      pins: Array.isArray(component.pins) ? component.pins : [], 
      // Ensure properties is an object or null
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[700px] max-h-[90vh]">
        ...
        <ScrollArea className="h-full max-h-[60vh] pr-4">
          {!isValid ? (
            <div className="p-4 border border-red-200 bg-red-50 rounded-md flex items-center gap-2 text-red-600">
              <AlertCircle className="h-5 w-5" />
              <span>Invalid component data provided. Unable to display details.</span>
            </div>
          ) : (
              ...
              {component.type && (
                <div>
                  ...
                  <div className="border rounded-md p-4 bg-gray-50 flex justify-center items-center" style={{ minHeight: "200px" }}>
                    <EnhancedComponentPreview component={safeComponent} />
                  </div>
                </div>
              )}
